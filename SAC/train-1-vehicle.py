...
while True:
    ...
    for veh_id, vehicle_data in batch_data.items():
        current_power = max(min(vehicle_data['transmissionPower'], 20.0), 1.0)
        current_beacon = max(min(vehicle_data['beaconRate'], 10.0), 1.0)
        ...
        if veh_id == "veh1":
            if timestamp > 0 and veh_id in prev_states:
                ...
                agent.store_transition(prev_states[veh_id], prev_actions[veh_id], reward, current_state, done)
            
            prev_states[veh_id] = current_state
    
    # TRAINING THE PREVIOUS TIMESTAMP
    if timestamp > 0 and "veh1" in prev_states:
        agent.train()
    ...
    for veh_id, vehicle_data in batch_data.items():
        ...
        if veh_id == "veh1":
            ...
            # normalized actions
            new_power = (action[0] + 1) / 2 * (20 - 10) + 10
            new_beacon = (action[1] + 1) / 2 * (10 - 10) + 10
            ...
            # Untuk kendaraan selain veh1, gunakan nilai lama (tidak diubah)
