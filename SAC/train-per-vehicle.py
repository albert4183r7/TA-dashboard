...
while True:
    ...
    for veh_id, vehicle_data in batch_data.items():
        ...
        if timestamp > 0 and veh_id in prev_states:
            ...
            agent.add_experience(veh_id, prev_states[veh_id], prev_actions[veh_id], reward, current_state, done)

        prev_states[veh_id] = current_state

    # TRAINING PER VEHICLE
    for veh_id in batch_data.keys():
        if agent.has_enough_data(veh_id):
            agent.train_for_vehicle(veh_id)

            # Save latest model and buffers
            agent.save_all_buffers()
            logging.info(f"Replay buffers saved.")
            torch.save(agent, LATEST_MODEL_PATH)
            logging.info(f"Latest model saved to {LATEST_MODEL_PATH}")

            # Record loss
            write_record_loss(timestamp, agent.policy_loss, agent.q1_loss, agent.q2_loss, agent.alpha_loss)
